...
bool MApp::OnInit()
{
    ...
    gluPerspective(45.0f,(GLdouble)W_WIDTH/W_HEIGHT,0.1f,100.0f);

    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();

    //setup physics world (must change this "//")
    glTranslatef(-1.5f,0.0f,-6.0f); //
    glTranslatef(3.0f,0.0f,0.0f); //
    float coiso[16]; //
    glGetFloatv(GL_MODELVIEW_MATRIX, coiso); //
    c->setOpenglMat(&coiso[0]); //

    glTranslatef(-1.f,3.0f,0.f); //
    glGetFloatv(GL_MODELVIEW_MATRIX, coiso); //
    p->setOpenglMat(&coiso[0]); //
...
void MApp::DrawRoomObj()
{
    if(!objModel)
    {
        ...
        //objModel = glmReadOBJ("../Resources/Sala/sala1.obj");

        if(!objModel){printf("Failed to load room model!\n"); exit(0);}
