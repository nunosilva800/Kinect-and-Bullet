...
*/
Cube::Cube() :
halfExtention(Math::newVector3(1,1,1))
{
    type = CUBE;
...
halfExtention(Math::newVector3(lado,lado,lado))
{
    type = CUBE;
    Math::identity4(this->objTransMat);
    this->setInitialColors();
...
{
    float size = ((v1->x - v2->x) < 0) ? -(v1->x - v2->x) : (v1->x - v2->x);
    halfExtention = Math::newVector3(size,size,size);
    type = CUBE;
    Math::identity4(this->objTransMat);
    this->setInitialColors();
    this->contructCube(size);
}
