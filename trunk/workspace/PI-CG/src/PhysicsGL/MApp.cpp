...
// strange way to do model loading and rendering
// but i was not able to get GLM to work properly (linker error, wtf!?!?)
GLMmodel * objModel;
GLuint text;


/*
 * Convert an IplImage to an OpenGL image
 */
int loadTexture_Ipl(){
...
//==============================================================================
MApp::MApp() :
	Surf_Display(0), lastTime(0) {
	c = new Cube();
	p = new Paralelepipedo(0.5, 0.5, 0.5);
	e = new Esfera();
	bulletWorld = new MPhysicsWorld();
	Running = true;
...
bool MApp::OnInit() {
	...
	//configure SDL display
	if ((Surf_Display = SDL_SetVideoMode(W_WIDTH, W_HEIGHT, 32, SDL_HWSURFACE
...
//	gluPerspective(45.0f, (GLdouble) W_WIDTH / W_HEIGHT, 1.0f, 1000.0f);
	glOrtho(-2.5, 2.5, -1.3, 1.3, 0, 5);

	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();

	//setup physics world
	c->setInitialPosition(1.5f,0.0f,-6.0f);
	p->setInitialPosition(0.5f,3.0f,-6.0f);
	//c2->setInitialPosition(2.5f,8.0f,-7.0f);
	e->setInitialPosition(0.0f,0.0f,-4.9f);

//	bulletWorld->addBody((Primitiva*)e);

// SET texture for camera frame
	...
	bulletWorld->init(); //build physical world

	//add bodies here
//	bulletWorld->addBody((Primitiva*) c);
//	bulletWorld->addBody((Primitiva*) p);
//	bulletWorld->addBody((Primitiva*) e);
...
;
//------------------------------------------------------------------------------
void MApp::DrawRoomObj() {
	if (!objModel)
	{
		objModel = new GLMmodel();
		objModel
				= glmReadOBJ(
						"/home/rr/Desktop/PI_CG/trunk/workspace/PI-CG/Debug/Resources/Sala/sala1.obj");
		if (!objModel)
		{
			printf("Failed to load room model!\n");
			exit(0);
		}
		// Depending on your project you might want to keep the original size
		//and positions you had in 3DS Max or GMAX so you may have to comment this.
		//glmUnitize(objModel);
...
//------------------------------------------------------------------------------
int _frame = 0, _time, _timebase = 0;
void MApp::OnLoop() {
	int currentTime = SDL_GetTicks();
	//update physics world
	bulletWorld->update((float) (currentTime - lastTime) * 0.001f);
...
void MApp::OnRender() {
	...
	// Get image from OpenCV and put it on a texture
	switch (source)
	{
		case CAP_FROM_FILE:
			captureIplImage();
	 ...
	 */
	GLdouble eyeX = 0.0f;
	GLdouble eyeY = 2.75f / 2;
	GLdouble eyeZ = 2.4999f;
...
//		GLdouble eyeZ = 10.f;
	...
	GLdouble camX = 0.0f;
	GLdouble camY = 2.75f / 2;
	GLdouble camZ = 0.0f;

	gluLookAt(eyeX, eyeY, eyeZ, camX, camY, camZ, 0, 1, 0);

	GLdouble fc_x = 0;		//deltaX - no need 2 change
	GLdouble fc_y = 2.75/2;	//deltaY - half the heigth of the room
	GLdouble fc_z = -2.5;	//deltaZ - half of the depth of the room

	float W_HEIGHT_ = 2.75;
	float W_WIDTH_  = 5;


	/*
	 * project video frame into the view frustum far plane
	 */
	...
	glBegin(GL_QUADS);
	glTexCoord2f(0, 0);
	glVertex3f(fc_x - W_WIDTH_ / 2, fc_y - W_HEIGHT_ / 2, fc_z);
	...
	glLoadIdentity();
	glPointSize(3.0f);
	glTranslatef(-2.5, 2.75/2, 0);

	double divx = 640 * 0.2;
	double divy = -480 * 1/2.75;
	std::vector<DetectedObject>* detectedObjs = getDetectedObjects();
	for (int i = 0; i < detectedObjs->size(); i++)
	{
		...
		{
			glVertex3f(points[j].x/divx, points[j].y/divy, -points[j].z);
		}
		glEnd();

		//drawing axis aligned bounding rectangle (z value is the value of 1st point on silhouete (root))
		CvRect bRect = detectedObjs->at(i).bRect;

//		Paralelepipedo *ppBox = new Paralelepipedo(
//				Math::newVector3(bRect.x/divx + bRect.x/divx + bRect.width/divx, bRect.y/divy, -points[0].z),
//				Math::newVector3(bRect.x/divx, bRect.y/divy - bRect.height/divy, -points[0].z-0.3));
//		Paralelepipedo *ppBox = new Paralelepipedo(bRect.width/(2.0*divx) , bRect.height/(2.0*divy),0.15);
//		ppBox->setInitialPosition(
//				(bRect.x)/(divx),
//				(bRect.y)/(divy),
//				-points[0].z-0.15 );
//		ppBox->Draw();

		GLfloat HUMAN_DEPTH = 0.3;

		glBegin(GL_QUADS);
						glVertex3f(bRect.x/divx, bRect.y/divy, -points[0].z);
						glVertex3f(bRect.x/divx, bRect.y/divy + bRect.height/divy, -points[0].z);
						glVertex3f(bRect.x/divx + bRect.width/divx, bRect.y/divy + bRect.height/divy, -points[0].z);
						glVertex3f(bRect.x/divx + bRect.width/divx, bRect.y/divy, -points[0].z);
					glEnd();

					glTranslated(0.05,0.05,0.00);

					glBegin(GL_QUADS);
						glVertex3f(bRect.x/divx, bRect.y/divy, -points[0].z+ HUMAN_DEPTH);
						glVertex3f(bRect.x/divx, bRect.y/divy + bRect.height/divy, -points[0].z+ HUMAN_DEPTH);
						glVertex3f(bRect.x/divx + bRect.width/divx, bRect.y/divy + bRect.height/divy, -points[0].z + HUMAN_DEPTH);
						glVertex3f(bRect.x/divx + bRect.width/divx, bRect.y/divy, -points[0].z+ HUMAN_DEPTH);
					glEnd();
//
//
//
//		double altura = (double)bRect.height / fabs(divy);
//		double largura = (double) bRect.width / fabs(divx);
//		double profundidade = HUMAN_DEPTH;
//
//		Paralelepipedo *ppBox = new Paralelepipedo(largura,altura,profundidade);
//
//		ppBox->setInitialPosition(
//				bRect.x/divx + largura /2,
//				 bRect.y/divy + altura /2, -points[0].z - profundidade / 2
//					 );

		//ppBox->setInitialPosition(0,0,-points[0].z - profundidade / 2 );
...
void MApp::OnLButtonDown(int mX, int mY) {
	bulletWorld->getPhysicsWorld()->setGravity(btVector3(0, 2, 0));
}
;
//==============================================================================
...
static GLvoid glmFirstPass(GLMmodel* model, FILE* file) {
	...
	group = model->groups;
	while (group) {
		group->triangles = (GLuint*) malloc(sizeof(GLuint)
				* group->numtriangles);
		group->numtriangles = 0;
		group = group->next;
	}
		...
		case 'v': /* v, vn, vt */
			...
			case '\0': /* vertex */
				fscanf(file, "%f %f %f", &vertices[3 * numvertices + 0],
						&vertices[3 * numvertices + 1], &vertices[3
								* numvertices + 2]);
				numvertices++;
				break;
			case 'n': /* normal */
				fscanf(file, "%f %f %f", &normals[3 * numnormals + 0],
						&normals[3 * numnormals + 1], &normals[3 * numnormals
								+ 2]);
				numnormals++;
				break;
			case 't': /* texcoord */
...
GLvoid glmFacetNormals(GLMmodel* model) {
	...
	model->facetnorms = (GLfloat*) malloc(sizeof(GLfloat) * 3
			* (model->numfacetnorms + 1));

	for (i = 0; i < model->numtriangles; i++) {
		model->triangles[i].findex = i + 1;

		u[0] = model->vertices[3 * T(i).vindices[1] + 0] - model->vertices[3
				* T(i).vindices[0] + 0];
		u[1] = model->vertices[3 * T(i).vindices[1] + 1] - model->vertices[3
				* T(i).vindices[0] + 1];
		u[2] = model->vertices[3 * T(i).vindices[1] + 2] - model->vertices[3
				* T(i).vindices[0] + 2];

		v[0] = model->vertices[3 * T(i).vindices[2] + 0] - model->vertices[3
				* T(i).vindices[0] + 0];
		v[1] = model->vertices[3 * T(i).vindices[2] + 1] - model->vertices[3
				* T(i).vindices[0] + 1];
		v[2] = model->vertices[3 * T(i).vindices[2] + 2] - model->vertices[3
				* T(i).vindices[0] + 2];
...
GLvoid glmVertexNormals(GLMmodel* model, GLfloat angle) {
	...
	model->normals = (GLfloat*) malloc(sizeof(GLfloat) * 3 * (model->numnormals
			+ 1));
	...
	for (i = 1; i <= model->numvertices; i++) {
		...
		while (node) {
			if (node->averaged) {
				...
				model->normals[3 * numnormals + 0] = model->facetnorms[3
						* T(node->index).findex + 0];
				model->normals[3 * numnormals + 1] = model->facetnorms[3
						* T(node->index).findex + 1];
				model->normals[3 * numnormals + 2] = model->facetnorms[3
						* T(node->index).findex + 2];
				if (T(node->index).vindices[0] == i)
...
GLvoid glmLinearTexture(GLMmodel* model) {
	...
	model->texcoords = (GLfloat*) malloc(sizeof(GLfloat) * 2
			* (model->numtexcoords + 1));

	glmDimensions(model, dimensions);
	scalefactor = 2.0 / glmAbs(glmMax(glmMax(dimensions[0], dimensions[1]),
			dimensions[2]));
...
glmReadOBJ(char* filename) {
	...
	/* allocate memory */
	model->vertices = (GLfloat*) malloc(sizeof(GLfloat) * 3
			* (model->numvertices + 1));
	model->triangles = (GLMtriangle*) malloc(sizeof(GLMtriangle)
			* model->numtriangles);
	if (model->numnormals) {
		model->normals = (GLfloat*) malloc(sizeof(GLfloat) * 3
				* (model->numnormals + 1));
	}
	if (model->numtexcoords) {
		model->texcoords = (GLfloat*) malloc(sizeof(GLfloat) * 2
