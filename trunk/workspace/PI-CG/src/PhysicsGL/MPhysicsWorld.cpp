...
void MPhysicsWorld::addBody(Primitiva* cub)
{
    ...
    case ESFERA:
        {
			Esfera* esf = (Esfera*) cub;
			fallShape = new btSphereShape(esf->getRaio());
			break;
        }
    };
 ...
 void MPhysicsWorld::addPessoa(Pessoa *p)
 {
	std::vector<Primitiva*>::iterator it = p->getPrimitivas()->begin();

	for(std::vector<Primitiva*>::iterator it = p->getPrimitivas()->begin(); it != p->getPrimitivas()->end() ; it++)
	{
		this->addBody(*it);
	}
} ;
